...
**Implementation**: Before building a whitelist, the core `is_admin()` decorator must be rewritten to check against an integer `user.id`. The new `dm_whitelist` table should strictly store and validate Discord User IDs, not string usernames, as name-based verification is highly vulnerable to spoofing. Create a new Discord command (e.g., `/whitelist add <user_id>`) that allows the admin to update this list dynamically, keeping it consistent with the rest of the bot's state management logic without requiring a reboot.
...
**Implementation**: Export the CPU-optimized Q4_K_M GGUF model artifact. Do not rewrite the Discord bot to import `llama.cpp` natively, as that will block the asyncio event loop and crash the bot. Instead, run LM Studio independently on the laptop, load the Q4_K_M GGUF there, and point the bot's `.env` configuration to that local LM Studio server URL. Log the tokens-per-second and RAM footprint metrics.
...
**Implementation**: Implement a graceful shutdown mechanism for the `/restart` and `/update` commands. Set a `shutting_down` flag that rejects new messages, waits for the `global_llm_lock` to clear the current queue natively, and only executes `os.execv` once the queue is empty. For unexpected hardware crashes, reconstruct the message solely from the SQLite `formatted_input` text without attempting to fetch the original Discord message object. This approach strictly prevents triggering severe Discord API rate limiting.
...
**Implementation**: Create a designated `/update` application command. Set an internal state flag to block new messages from entering the active processing loop, routing them directly to the persistent queue instead. Wait for the `global_llm_lock` to clear natively. The `git pull` command must be wrapped in `asyncio.create_subprocess_shell()` to ensure the bot maintains its heartbeat with Discord while fetching the updates from GitHub. Finally, use `os.execv` to reboot the bot script automatically.
...
**Implementation**: Detect when the history hits the `max_history` capacity threshold. Extract the oldest messages targeted for deletion. Contextual summarization must be scheduled into the exact same synchronous queue as standard user messages and acquire the `global_llm_lock` just like a normal response. Alternatively, offload the summarization task to a completely separate, ultra-lightweight CPU-only endpoint so it does not compete for the primary GPU VRAM. Prepend the resulting summary string into the persistent system prompt.
